...
`okurrr!` solves this annoyance in 1 line:
...
- 💅 **`okurr!` Alias**: Re-exported with 2 r's for backwards compatibility.
...
```rust
let val = okurrr!(res, err => return Err(err.into()));
let item = okurrr!(res, else err => { log::error!("{err}"); break; });
```

### 2. Fallback Expression
```rust
let count = okurrr!(res, else 0);
let count = okurrr!(res, 0); // 2-arg shorthand
```

### 3. Early Return Shorthand (`Result`)
```rust
let val = okurrr!(res); // Returns Err(err.into()) on failure
```

### 4. `Option<T>` Pattern (`okurrr_some!`)
```rust
let val = okurrr_some!(opt); // Returns None on failure
let val = okurrr_some!(opt, else "default_val");
```
...
When enabled, any caught `Err(err)` will automatically record a `tracing::error!(target: "okurrr", error = %err, ...)` event before executing your handler block!
